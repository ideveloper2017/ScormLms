...
type AcademicPeriodView = "all" | "years" | "semesters";

export function AdminAcademicPeriods({ view = "all" }: { view?: AcademicPeriodView }) {
  const { user } = useAuth();
  ...
  const pageTitle = view === "years" ? "O'quv yillari" : view === "semesters" ? "Semestrlar" : "O'quv yili va semestrlar";

  return <div className="space-y-6 p-3 sm:p-6">
    {(years.isPending || semesters.isPending) && <p role="status">O‘quv davrlari yuklanmoqda...</p>}
    {(years.isError || semesters.isError) && <div role="alert" className="rounded border p-3 text-destructive">O‘quv davrlarini yuklab bo‘lmadi. <Button variant="outline" onClick={() => void refresh()}>Qayta urinish</Button></div>}
    <div><h1 className="text-2xl font-bold">{pageTitle}</h1><p className="text-sm text-muted-foreground">Ta'lim jarayoni uchun boshqariladigan davr katalogi. Eski yozuvlar tarixiy snapshot sifatida o'zgarmaydi.</p></div>
    {view !== "semesters" && canWrite && <Card><CardHeader><CardTitle className="flex items-center gap-2"><CalendarRange className="h-5 w-5" />Yangi o'quv yili</CardTitle><CardDescription>Kod ketma-ket YYYY-YYYY formatida; muddat 1-sentabrdan 31-avgustgacha avtomatik olinadi.</CardDescription></CardHeader><CardContent className="flex flex-col gap-3 sm:flex-row sm:items-end"><div className="space-y-2"><Label>O'quv yili kodi</Label><Input className="w-44" value={yearCode} onChange={(event) => setYearCode(event.target.value)} placeholder="2027-2028" /></div><Button disabled={!/^\d{4}-\d{4}$/.test(yearCode) || createYear.isPending} onClick={() => createYear.mutate()}><Plus className="mr-2 h-4 w-4" />Yaratish</Button></CardContent></Card>}
    {view !== "semesters" && <Card><CardHeader><CardTitle>O'quv yillari</CardTitle><CardDescription>Bir dona joriy yil belgilanadi. Joriy yil faolsizlantirilmaydi.</CardDescription></CardHeader><CardContent className="space-y-3">{(years.data ?? []).map((year) => <div key={year.id} className="flex flex-col justify-between gap-3 rounded-md border p-3 sm:flex-row sm:items-center"><div><p className="font-medium">{year.code} {year.current && <CheckCircle2 className="ml-1 inline h-4 w-4 text-emerald-600" />}</p><p className="text-xs text-muted-foreground">{year.startsOn} — {year.endsOn}</p></div><div className="flex items-center gap-2"><Badge variant={year.active ? "secondary" : "outline"}>{year.active ? "Faol" : "Nofaol"}</Badge>{canWrite && !year.current && <><Button size="sm" variant="outline" onClick={() => updateYear.mutate({ id: year.id, active: true, current: true })}>Joriy qilish</Button><Button size="sm" variant="ghost" onClick={() => updateYear.mutate({ id: year.id, active: !year.active, current: false })}>{year.active ? "Faolsizlantirish" : "Faollashtirish"}</Button></>}</div></div>)}{years.data?.length === 0 && <p className="py-6 text-center text-muted-foreground">O'quv yili topilmadi.</p>}</CardContent></Card>}
    {view !== "years" && <Card><CardHeader><CardTitle>Semestrlar</CardTitle><CardDescription>Semestr raqami va kurs bog'lanishi o'zgarmaydi; faqat ishlatilish holati boshqariladi.</CardDescription></CardHeader><CardContent className="grid gap-3 md:grid-cols-2">{(semesters.data ?? []).map((semester) => <div key={semester.id} className="flex items-center justify-between rounded-md border p-3"><div><p className="font-medium">{semester.nameUz}</p><p className="text-xs text-muted-foreground">{semester.courseNumber}-kurs</p></div><div className="flex items-center gap-2"><Badge variant={semester.active ? "secondary" : "outline"}>{semester.active ? "Faol" : "Nofaol"}</Badge>{canWrite && <Button size="sm" variant="ghost" onClick={() => updateSemester.mutate({ id: semester.id, nameUz: semester.nameUz, active: !semester.active })}>{semester.active ? "O'chirish" : "Yoqish"}</Button>}</div></div>)}{semesters.data?.length === 0 && <p className="py-8 text-center text-muted-foreground">Semestr topilmadi.</p>}</CardContent></Card>}
  </div>;
}
