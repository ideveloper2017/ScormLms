...
const blank = (): SaveBiometricPolicyInput => ({
  ...
  faceTemplateRetentionDays: 30, proctoringEvidenceRetentionDays: 180, localMonitoringEnabled: false,
});
...
export function AdminBiometricGovernance() {
  ...
  return <div className="space-y-6 p-3 sm:p-6">
    ...
    {canWrite && <Card><CardHeader><CardTitle>{editingId ? "Qoralamani tahrirlash" : "Yangi siyosat qoralamasi"}</CardTitle><CardDescription>Muallifdan boshqa USER_MANAGE vakolatli xodim tasdiqlaydi.</CardDescription></CardHeader><CardContent className="grid gap-3 md:grid-cols-2">
      ...
      <label className="flex items-start gap-3 rounded border p-3 md:col-span-2"><input type="checkbox" checked={form.localMonitoringEnabled ?? false} onChange={e => setForm({ ...form, localMonitoringEnabled: e.target.checked })} /><span>Lokal tovush, yuz soni va bosh holati tahlilini yoqish. Audio/video brauzerda tahlil qilinadi, serverga faqat hodisa turi va vaqti yuboriladi. Bu imkoniyatni huquqiy asos, maxfiylik va rozilik matnida ko'rsating; amaldagi siyosat uchun yangi versiya va qayta rozilik kerak.</span></label>
