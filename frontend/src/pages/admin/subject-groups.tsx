...
export function AdminSubjectGroups() {
  ...
  const create = useMutation({
    ...
    onSuccess: async (created) => {
      ...
      setForm(emptyForm);
      await refresh();
      toast({ title: "Fan guruhi yaratildi" });
    },
    onError: fail,
  });
  ...
  return <div className="space-y-6 p-3 sm:p-6">
    ...
    {canWrite && selectedCurriculum && <Card><CardHeader><CardTitle>Yangi fan guruhi</CardTitle><CardDescription>Yil, semestr, dastur va fan tanlangan curriculum bandidan avtomatik olinadi.</CardDescription></CardHeader><CardContent className="grid gap-3 md:grid-cols-2">
      ...
      <div className="flex items-end"><Button disabled={!form.curriculumSubjectId || form.code.trim().length < 2 || form.name.trim().length < 3 || form.capacity < 1 || create.isPending} onClick={() => create.mutate()}><Plus className="mr-2 h-4 w-4" />Fan guruhini yaratish</Button></div>
    </CardContent></Card>}

    <div className="grid gap-4 lg:grid-cols-2">
      <Card><CardHeader><CardTitle>Fan guruhlari</CardTitle><CardDescription>Mavjud akademik guruhlar bu ro'yxatga avtomatik kiritilmaydi.</CardDescription></CardHeader><CardContent className="space-y-3">{(groups.data ?? []).map((group) => <button type="button" key={group.id} onClick={() => setSelectedGroupId(group.id)} className={`w-full rounded-md border p-3 text-left transition-colors ${selectedGroupId === group.id ? "border-primary bg-primary/5" : "hover:bg-muted/50"}`}><div className="flex items-start justify-between gap-3"><div><p className="font-medium">{group.code} · {group.name}</p><p className="text-sm text-muted-foreground">{group.programName} · {group.academicYear} · {group.semester}-semestr</p><p className="text-xs text-muted-foreground">{group.subjectCode} · {group.subjectName}</p></div><div className="flex flex-col items-end gap-1"><Badge variant={group.active ? "secondary" : "outline"}>{group.active ? "Faol" : "Nofaol"}</Badge><span className="text-xs">{group.memberCount}/{group.capacity}</span></div></div></button>)}{groups.data?.length === 0 && <p className="py-8 text-center text-muted-foreground">Fan guruhi hali yaratilmagan.</p>}</CardContent></Card>
