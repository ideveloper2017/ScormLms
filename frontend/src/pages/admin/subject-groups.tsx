...
export function AdminSubjectGroups({ lockedCurriculumId }: AdminSubjectGroupsProps = {}) {
  ...
  useEffect(() => { setCandidatePage(0); }, [selectedGroupId, candidateSearch]);
  ...
  const candidates = useQuery({
    queryKey: ["subject-groups", selectedGroupId, "candidates", candidateSearch, candidatePage],
    queryFn: () => subjectGroupApi.candidates(selectedGroupId!, { search: candidateSearch.trim() || undefined, page: candidatePage, size: 20 }),
  ...
  useEffect(() => { if (candidates.data && candidatePage > 0 && candidatePage >= candidates.data.totalPages) setCandidatePage(Math.max(0, candidates.data.totalPages - 1)); }, [candidates.data, candidatePage]);
  ...
  return <div className={embedded ? "space-y-6" : "space-y-6 p-3 sm:p-6"}>
    ...
    {canWrite && selectedCurriculum && <Card><CardHeader><CardTitle>Yangi fan guruhi</CardTitle><CardDescription>Yil, semestr, dastur va fan tanlangan curriculum bandidan avtomatik olinadi.</CardDescription></CardHeader><CardContent className="grid gap-3 md:grid-cols-2">
      ...
      <div className="flex items-end"><Button disabled={!form.curriculumSubjectId || form.code.trim().length < 2 || form.name.trim().length < 3 || !Number.isInteger(form.capacity) || form.capacity < 1 || form.capacity > 500 || busy} onClick={() => create.mutate()}><Plus className="mr-2 h-4 w-4" />Fan oqimini yaratish</Button></div>
    </CardContent></Card>}

    <div className="grid gap-4 lg:grid-cols-2">
      <Card><CardHeader><CardTitle>Fan oqimlari</CardTitle><CardDescription>Mavjud asosiy guruhlar bu ro'yxatga avtomatik kiritilmaydi.</CardDescription></CardHeader><CardContent className="space-y-3">{(groups.data ?? []).map((group) => <button type="button" key={group.id} disabled={busy} onClick={() => setSelectedGroupId(group.id)} className={`w-full rounded-md border p-3 text-left transition-colors ${selectedGroupId === group.id ? "border-primary bg-primary/5" : "hover:bg-muted/50"}`}><div className="flex items-start justify-between gap-3"><div><p className="font-medium">{group.code} · {group.name}</p><p className="text-sm text-muted-foreground">{group.programName} · {group.academicYear} · {group.semester}-semestr</p><p className="text-xs text-muted-foreground">{group.subjectCode} · {group.subjectName}</p></div><div className="flex flex-col items-end gap-1"><Badge variant={group.active ? "secondary" : "outline"}>{group.active ? "Faol" : "Nofaol"}</Badge><span className="text-xs">{group.memberCount}/{group.capacity}</span></div></div></button>)}{groups.data?.length === 0 && <p className="py-8 text-center text-muted-foreground">Fan oqimi hali yaratilmagan.</p>}</CardContent></Card>
      ...
      <Card><CardHeader><CardTitle className="flex items-center gap-2"><Users className="h-5 w-5" />{selectedGroup ? `${selectedGroup.code} talabalari` : "Talabalarni biriktirish"}</CardTitle><CardDescription>{selectedGroup ? "Faqat shu dastur, o'quv yili va semestrdagi ACTIVE talabalar ko'rsatiladi." : "Boshqarish uchun chap tomondan fan guruhini tanlang."}</CardDescription></CardHeader>{selectedGroup && <CardContent className="space-y-5">
        ...
        {selectedGroup.active && <div><h3 className="mb-2 text-sm font-semibold">Mos talabalar</h3><div className="relative mb-2"><Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" /><Input className="pl-9" aria-label="Fan oqimiga talaba qidirish" disabled={busy} value={candidateSearch} onChange={(event) => setCandidateSearch(event.target.value)} placeholder="F.I.Sh. yoki talaba raqami" /></div><div className="space-y-2">{candidates.isFetching && <p role="status">Talabalar yuklanmoqda...</p>}{(candidates.data?.items ?? []).map((student) => <div key={student.studentId} className="flex items-center justify-between rounded-md border p-2"><div><p className="text-sm font-medium">{student.fullName}</p><p className="text-xs text-muted-foreground">{student.studentNumber} · {student.semesterNumber}-semestr</p></div>{canWrite && <Button size="sm" variant="outline" disabled={assign.isPending || selectedGroup.memberCount >= selectedGroup.capacity} onClick={() => assign.mutate(student.studentId)}><UserPlus className="mr-1 h-4 w-4" />Biriktirish</Button>}</div>)}{candidates.isSuccess && candidates.data.items.length === 0 && <p className="text-sm text-muted-foreground">Mos bo'sh talaba topilmadi.</p>}{candidates.data && <div className="flex flex-wrap items-center justify-between gap-2 pt-3 text-sm"><span>{candidates.data.totalElements} talaba · {candidatePage + 1}/{Math.max(1, candidates.data.totalPages)}-sahifa</span><div className="flex gap-2"><Button size="sm" variant="outline" disabled={busy || candidates.isFetching || candidatePage === 0} onClick={() => setCandidatePage(candidatePage - 1)}>Oldingi</Button><Button size="sm" variant="outline" disabled={busy || candidates.isFetching || candidatePage + 1 >= candidates.data.totalPages} onClick={() => setCandidatePage(candidatePage + 1)}>Keyingi</Button></div></div>}</div></div>}
