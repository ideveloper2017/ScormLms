...
export function TeacherExams() {
  ...
  return <div className="p-3 sm:p-6 space-y-6">
    ...
    {selected && <Card><CardHeader><CardTitle>{selected.title}: auditoriya qaydnomasi</CardTitle><CardDescription>Kelganlikni vakolatli xodim tasdiqlaydi; tasdiq vaqti serverda yoziladi.</CardDescription></CardHeader><CardContent className="space-y-3">{(attendance.data?.attendanceRecords ?? []).map(record => { const saved = resultByEnrollment.get(record.enrollmentId); return <div key={record.id} className="border rounded-lg p-3 grid md:grid-cols-[1fr_180px_120px_120px] gap-2 items-center"><div><div className="font-medium">{record.studentName}</div><div className="text-xs text-muted-foreground">{record.studentEmail} · {record.verificationTime ? `Tasdiqlagan: ${record.verifiedBy}` : 'Tasdiqlanmagan'}</div></div><Select value={attendanceDraft[record.enrollmentId] ?? record.status} onValueChange={value => setAttendanceDraft(old => ({ ...old, [record.enrollmentId]: value as ExamAttendanceStatus }))} disabled={selected.status !== 'ONGOING'}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="PRESENT">Qatnashdi</SelectItem><SelectItem value="LATE">Kechikdi</SelectItem><SelectItem value="ABSENT">Qatnashmadi</SelectItem><SelectItem value="EXCUSED">Sababli</SelectItem></SelectContent></Select><Button size="sm" variant="outline" disabled={selected.status !== 'ONGOING'} onClick={() => mark.mutate({ enrollmentId: record.enrollmentId, status: attendanceDraft[record.enrollmentId] ?? record.status })}>Tasdiqlash</Button><div className="flex gap-1"><Input type="number" min="0" max="100" value={scoreDraft[record.enrollmentId] ?? saved?.score ?? ''} onChange={e => setScoreDraft(old => ({ ...old, [record.enrollmentId]: e.target.value }))} placeholder="Ball" disabled={selected.status !== 'ONGOING' || !['PRESENT', 'LATE'].includes(record.status)} /><Button size="sm" disabled={selected.status !== 'ONGOING' || !scoreDraft[record.enrollmentId]} onClick={() => grade.mutate({ enrollmentId: record.enrollmentId, score: Number(scoreDraft[record.enrollmentId]) })}>{saved?.grade ?? 'Saqlash'}</Button></div></div>; })}{attendance.data?.attendanceRecords.length === 0 && <p className="text-center text-muted-foreground py-5">Ro'yxat sessiya e'lon qilinganda yaratiladi.</p>}</CardContent></Card>}
