    ...
    <div className="p-3 sm:p-4 md:p-6 space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-start gap-3">
        ...
        <div className="flex-1">
          ...
          <p className="text-sm text-muted-foreground">{course.subjectName || "Fan ko'rsatilmagan"} · {course.programName || "Dastur biriktirilmagan"} · {course.language || "til yo'q"} · {course.groupName || "Guruh biriktirilmagan"} · {activeStudents} talaba</p>
        </div>
      ...
      <Tabs defaultValue={initialTab}>
        <div className="overflow-x-auto"><TabsList className="grid min-w-[540px] w-full grid-cols-5"><TabsTrigger value="overview">Umumiy</TabsTrigger><TabsTrigger value="modules">Modullar</TabsTrigger><TabsTrigger value="contents">Kontent</TabsTrigger><TabsTrigger value="students">Talabalar</TabsTrigger><TabsTrigger value="forum">Forum</TabsTrigger></TabsList></div>
        ...
        <TabsContent value="contents" className="mt-4 space-y-3">
          <Card><CardHeader><CardTitle className="text-base">Oddiy kontent va kelib chiqish ma'lumotlari</CardTitle><CardDescription>Muallif, til, versiya, manba va amal qilish davri majburiy. Kontent tili kurs, fan dasturi va faol talabalar ta'lim tiliga mos bo'lishi shart.</CardDescription></CardHeader><CardContent className="grid grid-cols-1 md:grid-cols-2 gap-3">
          ...
          {contents.map(content => { const meta = CONTENT_META[content.contentType] ?? CONTENT_META.file; const Icon = meta.icon; const modulePublished = modules.find(item => item.id === content.moduleId)?.status === "published"; const validity = content.effective ? "Amalda" : content.validFrom > today() ? "Rejalashtirilgan" : "Muddati tugagan"; const reviewLabel = content.reviewStatus === "approved" ? "Tasdiqlangan" : content.reviewStatus === "in_review" ? "Ekspertizada" : content.reviewStatus === "changes_requested" ? "Tuzatishga qaytarilgan" : "Ekspertizaga yuborilmagan"; const compatible = content.compatibility.compatible; const compatibilityMessage = content.compatibility.issues.map(issue => issue.message).join("; "); return <Card key={content.id}><CardContent className="p-4 flex flex-col lg:flex-row lg:items-center gap-3"><Icon className={`h-5 w-5 ${meta.className}`} /><div className="flex-1 min-w-0"><p className="font-medium truncate">{content.title}</p><p className="text-xs text-muted-foreground truncate">{content.moduleTitle} · v{content.contentVersion} · {content.languageCode} · {content.authorName}</p><p className="text-xs text-muted-foreground truncate">{content.sourceName} · {content.validFrom} — {content.validUntil || "cheklanmagan"}</p>{!compatible && <p className="mt-2 text-xs text-destructive">{compatibilityMessage}</p>}</div><div className="flex items-center gap-2 flex-wrap"><Badge variant="outline">{meta.label}</Badge><Badge variant={content.effective ? "default" : "secondary"}>{validity}</Badge><Badge variant={compatible ? "default" : "destructive"}>{compatible ? "Til/dastur mos" : "Til/dastur nomos"}</Badge><Badge variant={content.reviewStatus === "approved" ? "default" : "secondary"}>{reviewLabel}</Badge><Badge variant={content.status === "published" ? "default" : "secondary"}>{content.status === "published" ? "Nashrda" : "Qoralama"}</Badge><Button variant="ghost" size="sm" className="gap-1" onClick={() => setHistoryContent(content)}><History className="h-4 w-4" />Tarix</Button>{content.status === "draft" && ["draft", "changes_requested"].includes(content.reviewStatus) && <Button variant="outline" size="sm" disabled={submitReviewMutation.isPending || !compatible} title={!compatible ? compatibilityMessage : undefined} onClick={() => submitReviewMutation.mutate(content.id)}>Ekspertizaga</Button>}<Button variant="ghost" size="icon" disabled={content.reviewStatus === "in_review"} onClick={() => editContent(content)}><Edit className="h-4 w-4" /></Button><Button variant="outline" size="sm" disabled={content.status === "draft" && (!modulePublished || content.reviewStatus !== "approved" || !compatible)} title={!compatible ? compatibilityMessage : content.reviewStatus !== "approved" ? "Avval mustaqil ekspert tasdig'ini oling" : !modulePublished ? "Avval modulni nashr qiling" : undefined} onClick={() => contentStatusMutation.mutate({ contentId: content.id, status: content.status === "published" ? "DRAFT" : "PUBLISHED" })}>{content.status === "published" ? "Yashirish" : "Nashr"}</Button><Button variant="ghost" size="icon" disabled={content.reviewStatus === "in_review"} className="text-destructive" onClick={() => deleteContentMutation.mutate(content.id)}><Trash2 className="h-4 w-4" /></Button></div></CardContent></Card>; })}
        </TabsContent>
