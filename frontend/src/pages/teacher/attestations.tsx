  ...
  return <div className="p-3 sm:p-6 space-y-6">
        ...
        {[1, 2].map(i => (
          <Card key={i} className="animate-pulse">
            <CardContent className="p-4 space-y-3">
              ...
              <div className="flex gap-2"><div className="h-8 bg-muted rounded w-24"></div></div>
            </CardContent>
          </Card>
        ))}
      </div>
    ...
    {!sessions.isLoading && sessions.data?.length === 0 && (
      <Card className="flex flex-col items-center justify-center p-8 text-center text-muted-foreground">
        ...
        <h3 className="font-semibold text-lg text-foreground">Attestatsiyalar yo'q</h3>
        <p>Hozircha birorta ham davlat attestatsiyasi ochilmagan.</p>
      </Card>
    )}
    {!sessions.isLoading && (sessions.data ?? []).length > 0 && (
      <div className="grid lg:grid-cols-2 gap-3">
        {(sessions.data ?? []).map(session => <Card key={session.id} className={selectedId === session.id ? 'border-primary' : ''}><CardContent className="p-4 space-y-3"><div className="flex justify-between"><div><div className="font-semibold">{session.title}</div><div className="text-sm text-muted-foreground">{session.courseTitle} · {session.examDate} {session.examTime} · {session.location}</div></div><Badge>{session.status}</Badge></div><div className="text-sm">Komissiya: {session.currentMemberCount}/{session.minCommissionMembers} · Himoya: {session.defenseCount}/{session.totalEnrolled} · O'tdi: {session.passedCount}</div><div className="flex flex-wrap gap-2"><Button size="sm" variant="outline" onClick={() => setSelectedId(session.id)}>Qaydnomani ochish</Button>{session.status === 'DRAFT' && <><Button size="sm" onClick={() => action.mutate({ id: session.id, kind: 'publish' })}>E'lon qilish</Button><Button size="icon" variant="ghost" onClick={() => action.mutate({ id: session.id, kind: 'remove' })}><Trash2 className="h-4 w-4" /></Button></>}{session.status === 'PUBLISHED' && <Button size="sm" onClick={() => action.mutate({ id: session.id, kind: 'start' })}><Play className="h-4 w-4 mr-1" />Boshlash</Button>}{session.status === 'ONGOING' && <Button size="sm" onClick={() => action.mutate({ id: session.id, kind: 'complete' })}><CheckCircle2 className="h-4 w-4 mr-1" />Yakunlash</Button>}</div></CardContent></Card>)}
      </div>
    )}
    {selected && <Card><CardHeader><CardTitle>{selected.title}: komissiya va himoyalar</CardTitle></CardHeader><CardContent className="space-y-4">{selected.status === 'DRAFT' && <div className="flex flex-col sm:flex-row gap-2"><Input type="number" value={memberId} onChange={e => setMemberId(e.target.value)} placeholder="Foydalanuvchi ID" /><Select value={memberRole} onValueChange={value => setMemberRole(value as typeof memberRole)}><SelectTrigger className="sm:w-48"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="MEMBER">A'zo</SelectItem><SelectItem value="SECRETARY">Kotib</SelectItem></SelectContent></Select><Button disabled={!memberId} onClick={() => addMember.mutate()}><UserPlus className="h-4 w-4 mr-1" />Qo'shish</Button></div>}
      ...
      {(detail.data?.defenseList ?? []).map(defense => <div key={defense.defenseId} className="border rounded-lg p-3 grid md:grid-cols-[1fr_auto_auto] gap-2 items-center"><div><div className="font-medium">{defense.studentName} {!defense.onsiteAttendanceRequired && <Badge variant="outline" className="ml-2">Xorijiy fuqaro — istisno</Badge>}</div><div className="text-xs text-muted-foreground">{defense.defenseStatus} · {defense.commissionDecision ?? 'qaror kutilmoqda'} · {defense.averageScore ?? 0} ball{defense.onsiteAttendanceRequired && defense.onsiteAttendanceConfirmedAt ? ' · shaxsan qatnashuv tasdiqlangan' : ''}</div></div><Button size="sm" variant="outline" disabled={selected.status !== 'ONGOING' || defense.defenseStatus === 'DEFENDED'} onClick={() => record.mutate({ id: defense.defenseId, onsiteAttendanceConfirmed: defense.onsiteAttendanceRequired })}>{defense.onsiteAttendanceRequired ? 'Shaxsan himoya tasdiqlandi' : 'Himoya yakunlandi'}</Button><div className="flex gap-1"><Input className="w-24" type="number" min="0" max="100" value={scores[defense.defenseId] ?? ''} onChange={e => setScores(old => ({ ...old, [defense.defenseId]: e.target.value }))} placeholder="Ball" /><Button size="sm" disabled={selected.status !== 'ONGOING' || defense.defenseStatus !== 'DEFENDED' || !scores[defense.defenseId]} onClick={() => grade.mutate({ defenseId: defense.defenseId, score: Number(scores[defense.defenseId]) })}>Baholash</Button>{protocol.data?.approved && defense.commissionDecision === 'PASS' && !defense.certificateIssued && <Button size="sm" onClick={() => certificate.mutate(defense.defenseId)}><Award className="h-4 w-4" /></Button>}</div></div>)}
