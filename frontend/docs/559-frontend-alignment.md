...
| Yakuniy 559 UAT qabuli | Dalil runi, 27 band reviewi va imzolangan protokol | `src/components/admin/decision-559-uat-panel.tsx`, `src/services/api/decision-559-uat-api.ts`, `src/pages/admin/compliance-559.tsx` | V45–V49 real API: run yaratish, yagona katalogdan band/baseline/mas'ul/blocker/manual qoldiq yo'riqnomasi, ko'p-faylli private dalil va mustaqil review, 27/27 evidence-ready bo'lgandagina canonical SHA va band natijalari oldindan yozilgan print-friendly `Imzolash uchun loyiha`, imzolangan PDF uploadi, evidence-set binding, submit/approve/reject, legacy schema-v2/v3 va schema-v4 audit manifesti hamda faqat `APPROVED` holatda barcha payload/`SHA256SUMS`li ZIP; `UAT_READ/WRITE/APPROVE`ga mos UI | 14 haqiqiy tashqi/manual dalilni mas'ullar kiritadi va boshqa vakolatli user review qiladi; V49 loyihasi PDFga chop etilib komissiya tomonidan imzolanadi va yuklanadi; final bundle/SHA/verifier hisoboti arxivlanmaguncha yakuniy qabul yopilmaydi |
